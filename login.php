...
try {
  ...
  $stmt = $conn->prepare('SELECT * FROM bdc_user WHERE email = :email AND password = :password');
...
<body>
	...
	<div id="container">
		...
		<hgroup id="login-title" class="large-margin-bottom">
			<h1>BDC Login</h1>
			<h5>&copy; TKMKT 2014</h5>
		</hgroup>

		<form method="post" action="" id="form-login">
			<ul class="inputs black-input large">
				<!-- The autocomplete="off" attributes is the only way to prevent webkit browsers from filling the inputs with yellow -->
				<li><span class="icon-mail mid-margin-right"></span><input type="text" name="email" id="login" value="" class="input-unstyled" placeholder="Login" autocomplete="off"></li>
	...
	<script>
		...
		notify('BDC Manager Login', 'Enter your login credentials below to begin your recorded session.', {
			autoClose: false,
			delay: 2500,
